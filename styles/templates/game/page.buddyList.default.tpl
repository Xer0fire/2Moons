		...
		<div class="card-body p-0">
			<div class="table-responsive">
				<table class="table table-striped table-sm">
						...
						<tr>
							...
							<td>
								{if $myBuddyRow.onlinetime < 4}
									<span class="text-green">{$LNG.bu_connected}</span>
								{elseif $myBuddyRow.onlinetime >= 4 && $myBuddyRow.onlinetime <= 15}
									<span class="text-yellow">{$myBuddyRow.onlinetime} {$LNG.bu_minutes}</span>
								{else}
									<span class="text-red">{$LNG.bu_disconnected}</span>
								{/if}
							</td>
							<td>
					...
					{foreachelse}
						<tr>
							<td colspan="6">{$LNG.bu_no_buddys}</td>
						</tr>
					{/foreach}
				</table>
			</div>
				...
				<div class="table-responsive">
					<table class="table table-striped table-sm">
						{if !empty($otherRequestList)}
							<tr>
								<th colspan="5">{$LNG.bu_requested}</th></tr>
							<tr>
								<th>{$LNG.bu_player}</th>
								<th>{$LNG.bu_alliance}</th>
								<th>{$LNG.bu_coords}</th>
							...
							{foreach $otherRequestList as $otherRequestID => $otherRequestRow}
								<tr>
									...
									<td>
										<a href="game.php?page=buddyList&amp;mode=accept&amp;id={$otherRequestID}"><i class="fa fa-check text-green"></i></a>
										<span class="mx-2">&nbsp;</span>
										<a href="game.php?page=buddyList&amp;mode=delete&amp;id={$otherRequestID}"><i class="fa fa-times text-red"></i></a>
									</td>
								</tr>
							{foreachelse}
								<tr>
						...
						{if !empty($myRequestList)}
							<tr>
								<th colspan="5">{$LNG.bu_my_requests}</th>
							</tr>
							<tr>
								<th>{$LNG.bu_player}</th>
								<th>{$LNG.bu_alliance}</th>
