...
<table class="table519">
	...
	<tr>
		...
		{if $DiploInfo}
			...
			{if !empty($DiploInfo.5)}<b><u>{$LNG.al_diplo_level.5}</u></b><br><br>{foreach item=PaktInfo from=$DiploInfo.5}<a href="?page=alliance&mode=info&amp;id={$PaktInfo.1}">{$PaktInfo.0}</a><br>{/foreach}<br>{/if}
		{else}
			{$LNG.al_no_diplo}
		{/if}
		</td>
