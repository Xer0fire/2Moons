						...
						<div class="row">
							<div class="col-md-12">
								...
								<div class="row content-center">
									<div class="col">
										<span class="float-sm-right">{$LNG.bd_max_ships}: {$Element.maxBuildable|number}</span>
										<p class="h5"><a href='#' onclick='info({$ID}, "{$LNG.tech.$ID}")' data-toggle='modal' data-target='#infoModal'>{$LNG.tech.{$ID}}</a> {if $Element.available != 0} ({$LNG.bd_available} {$Element.available|number}){/if}</p>
