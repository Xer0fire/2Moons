...
class VarsBuildCache implements BuildCache
{
	function buildCache()
	{
		...
		$reslist['tech']		= array();
		$reslist['fleet']		= array();
		$reslist['pfleet']		= array();
		$reslist['mfleet']		= array();
			...
			switch($varsRow['class']) {
				case 0: 
					...
					foreach($tmp as $type) {
						...
						if ($type == 1) {
						    $reslist['pbuild'][] = $varsRow['elementID'];
						}
						if ($type == 3) {
						    $reslist['mbuild'][] = $varsRow['elementID'];
						}
					}
				break;
				case 100: 
				...
				case 200: 
					...
					foreach($tmp as $type) {
					    if ($type == 1) {
					        $reslist['pfleet'][] = $varsRow['elementID'];
					    }
					    if ($type == 3) {
					        $reslist['mfleet'][] = $varsRow['elementID'];
					    }
					}
				break;
				case 400: 
